    ...
    def user_template_environment( user ):
        ...
        if user:
            ...
            if user.form_values_id:
                form_values = db_session.query(galaxy.model.FormValues).get(user.form_values_id)
                for i, field in enumerate(form_values.form_definition.fields):
                    ...
                    values[field_label] = value.strip()
            #------------------------------------------------------------------------------------------
            user_id = '%d' % user.id
            user_email = str( user.email )
            user_name = str( user.username )
            #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            user_omero_host = str(values.get('omero_host',None))
            user_omero_user = str(values.get('omero_user',None))
            user_omero_password = str(values.get('omero_password',None))
            api_key = db_session.query(galaxy.model.APIKeys).filter_by(user_id=user.id).first()
            if api_key:
                user_api_key = str(api_key.key)
            else:
            ...
            user_name = 'Anonymous'
            #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            user_omero_host = str(None)
            user_omero_user = str(None)
            user_omero_password = str(None)
            user_api_key = str(None)
            #------------------------------------------------------------------------------------------
        environment = {}
        ...
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        environment[ '__user_omero_host__' ] = environment[ 'user_omero_host' ] = user_omero_host
        environment[ '__user_omero_user__' ] = environment[ 'user_omero_user' ] = user_omero_user
        environment[ '__user_omero_password__' ] = environment[ 'user_omero_password' ] = user_omero_password
        environment[ '__user_api_key__' ] = environment[ 'user_api_key' ] = user_api_key
        #------------------------------------------------------------------------------------------  
        return environment
