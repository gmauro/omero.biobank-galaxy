...
class SelectToolParameter( ToolParameter ):
    ...
    def __init__( self, tool, input_source, context=None ):
        ...
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        #self.ajax = string_as_bool( elem.get( 'ajax' ) )
        #self.elem = elem
        self.ajax = string_as_bool( input_source.get( 'ajax' ) )
        self.elem = input_source
        #----------------------------------------------------------------------------------------------
    ...
    #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++   
    def get_ajax_options(self,trans,elem):
        import json,requests

        if trans.user:
            details = {}
            for i, field in enumerate(trans.user.values.form_definition.fields):
                field_label = field['label'].lower().replace(' ','_')
                try:
                    value = trans.user.values.content[field['name']]
                except KeyError:
                    value = form_values.content.get(field['label'], "")
                details[field_label] = value

            ajax_source = elem.get( "ajax-source", None )
            if not ajax_source: return list()

            url = details.get('omero_host',None)
            if not url: return list()
            url = 'http://%s:8080/%s' %(url.strip('/'),ajax_source.strip('/'))
            payload = {
                'ome_host' : details.get('omero_host',None),
                'ome_user' : details.get('omero_user',None),
                'ome_passwd' : details.get('omero_password',None)
            }
            ...
            if response['result'] not in ['None']:
                ...
                if string_as_bool(elem.get( 'sort' ) ):
                    #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    #return  sorted(result, key=lambda k: k['value'])
                    return  sorted(result, key=lambda k: k['label'].lower())
                    #----------------------------------------------------------------------------------------------
                return result
    ...
    def get_options( self, trans, other_values ):
        ...
        #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            ajax_options = list(self.static_options)
            for option in self.get_ajax_options(trans,self.elem):
                if len(self.static_options):  selected = string_as_bool( False )
                else: selected = string_as_bool( option.get( "selected", False ) )
                ajax_options.append( ( option['label'], option['value'], selected ) )
            return ajax_options
        #----------------------------------------------------------------------------------------------
        else:
    ...
    def get_legal_values( self, trans, other_values ):
        ...
        #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            return map( _get_UnvalidatedValue_value, set( v for _, v, _ in self.get_options( trans, other_values ) ) )
        #----------------------------------------------------------------------------------------------
        else:
...
class DrillDownSelectToolParameter( SelectToolParameter ):
    ...
    def get_ajax_options(self,trans,elem):
        if trans.user:
            if elem.get( "ajax-source", None) == 'orione/get/libraries_and_folders':
                ...
                restricted_library_ids = list(set([ lp.library_id for lp in trans.sa_session.query( trans.model.LibraryPermissions ) \
                                                                               .filter( trans.model.LibraryPermissions.table.c.action == library_access_action ) \
                                                                               .distinct() ]))
                accessible_restricted_library_ids = list(set([ lp.library_id for lp in trans.sa_session.query( trans.model.LibraryPermissions ).filter( and_( trans.model.LibraryPermissions.table.c.action == library_access_action,trans.model.LibraryPermissions.table.c.role_id.in_( current_user_role_ids ) ) ).distinct() ] ))
                #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                #active_libraries = trans.sa_session.query(trans.model.Library).filter( or_( not_( trans.model.Library.table.c.id.in_( restricted_library_ids ) ),
                active_libraries = trans.sa_session.query(trans.model.Library).filter( trans.model.Library.table.c.deleted == False ).filter( or_( not_( trans.model.Library.table.c.id.in_( restricted_library_ids ) ),
                                          trans.model.Library.table.c.id.in_( accessible_restricted_library_ids ) ) ).distinct(trans.model.Library.id)
                #----------------------------------------------------------------------------------------------
                options = list()
                for library in active_libraries:
                    sub_options = list()

                    for folder in library.get_active_folders(library.root_folder):
                        if folder.parent_id:
                            sub_options.append({'name' : folder.name, 'value' : "{0}?{1}".format(library.name.replace(' ','.'),folder.name.replace(' ','.')), 'selected' : False, 'options' : list()})
                    options.append({'name' : library.name, 'value' : str(library.id), 'selected' : False, 'options' : sub_options})
                #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                if string_as_bool(elem.get( 'sort' ) ):
                    return  sorted(options, key=lambda k: k['name'].lower())
                #----------------------------------------------------------------------------------------------
                return options
        return list()
    #----------------------------------------------------------------------------------------------
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            ajax_options = list(self.options)
            for option in self.get_ajax_options(trans,self.elem):
                ajax_options.append(option)
            return ajax_options
#----------------------------------------------------------------------------------------------
        return self.options
